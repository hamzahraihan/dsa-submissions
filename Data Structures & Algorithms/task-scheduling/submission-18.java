class Solution {
    public int leastInterval(char[] tasks, int n) {
        ...
        while(!maxHeap.isEmpty() || !q.isEmpty()){
            ...
            // decrease task each time increase if maxHeap is not empty
            if(!maxHeap.isEmpty()){
            ...
            // check if q still remain task, and task next available time reach time to process the task  
            // then add it to maxHeap so that task can be scheduled to process again
